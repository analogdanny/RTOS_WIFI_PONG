...
EUSCI_B_SPI_CLOCKSOURCE_SMCLK,             // SMCLK Clock Source
        24000000,                                   // SMCLK = DCO = 48MHZ
        12000000,                                    // SPICLK = 24Mhz
...
static void LCD_initSPI()
{
    ...
    /* Selecting P10.1 P10.2 and P10.3 in SPI mode */
    GPIO_setAsPeripheralModuleFunctionInputPin(
            GPIO_PORT_P10, GPIO_PIN1 | GPIO_PIN2 | GPIO_PIN3,
    ...
    // Init Chip Select Pins as outputs
...
//    //Set CLK, MOSI, LCD CS, and TP CS as output, MISO as input
...
//    UCB3CTLW0 = UCSWRST;
//
//    //Master Mode, SMCLK, Synchronous mode, Inactive state is high, MSB first
 ...
 *******************************************************************************/
void LCD_DrawRectangle(int16_t xStart, int16_t xEnd, int16_t yStart,
                       int16_t yEnd, uint16_t Color)
{
    ...
    uint32_t screen = (xEnd - xStart + 1) * (yEnd - yStart + 1);
 ...
 * Attention      : None
 *******************************************************************************/
inline void PutChar(uint16_t Xpos, uint16_t Ypos, uint8_t ASCI,
                    uint16_t charColor)
{
    uint16_t i, j;
...
void LCD_SetPoint(uint16_t Xpos, uint16_t Ypos, uint16_t color)
{
    ...
    if (Xpos > MAX_SCREEN_X || Xpos < MIN_SCREEN_X || Ypos > MAX_SCREEN_Y
            || Ypos < MIN_SCREEN_Y)
        return;
...
{
    /* Send out MSB */
    SPI_transmitData(EUSCI_B3_BASE, data >> 8);

    while (EUSCI_B_SPI_isBusy(EUSCI_B3_BASE))
        ;

    /* Send out LSB */
    SPI_transmitData(EUSCI_B3_BASE, data & 0xFF);
...
void LCD_Init(bool usingTP)
{
    ...
    if (usingTP)
    {
        P4->DIR &= ~BIT0;   // P4.0 direction set as input
...
Point TP_ReadXY()
{
    ...
    SPISendRecvByte(CHX);   // write to capture CHX data
    coord.x = SPISendRecvByte(CHX) << 5;    // MSB
    coord.x |= SPISendRecvByte(CHX) >> 3;         // LSB
    coord.x = coord.x * MAX_SCREEN_X / 4095;  // ADC -> pixel x
    ...
    SPISendRecvByte(CHY);   // write to capture CHY data
    coord.y = SPISendRecvByte(CHY) << 5;    // MSB
    coord.y |= SPISendRecvByte(CHY) >> 3;         // LSB
    coord.y = coord.y * MAX_SCREEN_Y / 4095;  // ADC -> pixel y
